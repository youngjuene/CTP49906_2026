# 실시간 피드백 지도

<p align="center">
  <a href="README.md">English</a> · <b>한국어</b>
</p>

CTP49906 *Multimodal AI* 워크숍용 강의실 도구입니다. 학생이 발표하는 동안 참석자들은
각자 휴대폰으로 링크를 열고 떠오른 생각을 적습니다. 최대 20,000 유니코드 코드 포인트의
응답 전체를 붙여 넣으면 원문을 먼저 저장합니다. 이후 의미 단위로 나누고 외부 API 호출 없이
**로컬에서** 임베딩·2D 투영을 마친 결과를 함께 공개합니다. 처리 시간은 길이와 대기열에
따라 달라지며, 화면에서 저장 완료·처리 중·지도 반영 완료를 구분합니다.

색은 **어느 학생에 대한 의견인지**, 모양은 **피드백 출처가 AI 생성인지 사람 작성인지**를
나타냅니다. 제출자의 신원과는 별개이며, AI가 의도된 기본값입니다. 사람이 쓴 피드백을
AI로 처리해도 출처는 바뀌지 않습니다. 이 세션은 모델이 주관적 경험을 어디까지 서술할 수 있는지를
묻고, 이 지도는 두 종류의 코멘트를 같은 의미 공간에 놓아 둘이 같은 자리에 떨어지는지를
눈으로 확인하게 합니다.

강의실 지도는 휴대폰·노트북·프로젝터에서 사용합니다. 추가 Embedding Atlas 분석 뷰어는
같은 필터와 선택 범위를 지원하기 전까지 비활성화합니다. 자동 의미 분할도 **아직 실제
수업 사용 기준을 통과하지 못했습니다.** [구현·출시 검증 기록](docs/qa/feedback-atlas-release.md)을 확인하세요.

명세는 [`realtime-feedback-atlas-prd.md`](realtime-feedback-atlas-prd.md)입니다.

## 이것이 아닌 것

로그인 시스템이 아닙니다. 비밀번호는 없습니다. 랜딩 페이지가 입력한 ID를 강사가 미리
등록한 명단과 대조하는 것이 전부입니다. 등록은 엄격합니다 — 명단에 없는 ID는 들어올 수
없습니다.

익명이 아니며, 익명인 척하지도 않습니다. `reviewer_id`는 서버에 저장되지만 참가자
공개 지도 데이터에는 포함되지 않습니다. 참가자에게는 본인의 신원과 소유 기록만 비공개로
전달됩니다. 고정 접근 코드로 들어가는 관리자 모드는 다른 제출자의 기록을 조회할 수 있습니다.

## 설치

[uv](https://docs.astral.sh/uv/)와 **Python 3.12 이상**이 필요합니다. 취향이 아니라
필요 조건입니다. numpy 2.5와 scipy 1.18이 모두 3.12를 요구하므로, 3.11 가상환경은 조용히
구버전으로 해석됩니다.

```bash
cd feedback-atlas
uv venv --python 3.12 --seed .venv
uv pip install --python .venv/bin/python -r requirements.txt
```

### 첫 실행 전에 Gemma 라이선스에 동의하세요

기본 모델 `google/embeddinggemma-300m`은 **게이트가 걸린** 다운로드입니다. 새로 클론한
저장소는 설치 시점이 아니라 모델을 불러오는 시점에 401 또는 403으로 실패합니다. 그러니
수업 당일이 아니라 미리 해두세요.

1. <https://huggingface.co/google/embeddinggemma-300m>에서 라이선스에 동의합니다.
2. **동의한 그 계정으로** 로그인합니다. `huggingface-cli login` 또는 `HF_TOKEN` 설정.
   라이선스에 동의한 계정과 다른 계정의 토큰은 토큰이 아예 없는 것과 똑같이 실패합니다.
3. 첫 실행이 1.2 GB 다운로드가 되지 않도록 가중치를 미리 받아둡니다.
   `hf download google/embeddinggemma-300m`

> **라이선스에 동의했는데도 계속 "gated"라고 나오면 `HF_TOKEN`을 확인하세요.**
> 환경 변수가 `huggingface-cli login`이 저장해 둔 토큰보다 우선하므로, 만료되었거나
> 잘못된 `HF_TOKEN`이 멀쩡한 로그인을 가려버립니다. 그리고 허브는 이를 gated 오류로
> 보고하기 때문에, 이미 모든 것이 정상으로 보이는 라이선스 페이지로 다시 보내집니다.
> `env -u HF_TOKEN python -m uvicorn …`으로 실행해 보면 둘을 가장 빨리 구분할 수
> 있습니다. 이 저장소의 첫 실행에서 실제로 시간을 잡아먹은 문제입니다.

게이트가 걸림돌이라면 `ATLAS_EMBEDDING_MODEL=e5-small-ko`가 있습니다. 게이트 없음,
Apache-2.0, 384차원입니다. 서버는 스스로 다른 모델로 넘어가지 않고 읽을 수 있는 오류와
함께 시작을 거부합니다. 아무도 고르지 않은 모델이 만든 지도는 지도가 없는 것보다 나쁩니다.

### 참가자 명단

수업 전에 등록하는 `roster.csv`입니다. [`roster.example.csv`](roster.example.csv) 참고.

```csv
id,display_name,role
kim.seoyeon,김서연,student
kang.minsu,강민수,auditor
ta.youngjune,영준,ta
```

역할은 세 가지입니다. `student`(수강생) 행은 **대상**이기도 합니다. 즉 피드백을 받을 수
있는 프로젝트입니다. `auditor`(청강생)와 `ta`(조교)는 의견을 쓰지만 대상이 되지는
않습니다. 수업의 실제 모습 그대로입니다 — 조교는 작업에 코멘트를 하지만, 조교의 작업에
코멘트하는 사람은 없습니다.

PRD 7의 표에는 `student`와 `auditor`만 있습니다. `ta`는 의도적으로 추가했습니다. PRD 3이
이미 조교를 의견을 제출할 수 있는 사용자로 적고 있고, 제출하려면 명단 항목이 필요하기
때문입니다. 고유한 역할이 없으면 조교를 청강생으로 등록해야 하는데, 그러면 귀속을 보라고
만든 관리자 패널이 조교를 있는 그대로 표시하지 못합니다. 대조는 공백·대소문자·전각 문자에 관대하고(한국어 IME가
전각 모드면 `ｋｉｍ`이 입력됩니다) 그 외에는 관대하지 않습니다. 편집 거리 기반 매칭은 한
학생의 ID가 다른 학생의 ID로 해석되게 만듭니다.

나중에 사람을 추가하려면 파일을 고치고 `SIGHUP`을 보냅니다. 재시작도, 연결 끊김도
없습니다.

```bash
kill -HUP $(pgrep -f 'uvicorn.*src.server')
```

### 환경 설정

[`.env.example`](.env.example)을 `.env`로 복사하세요. 기본값이 없는 필수 값이 두 개
있습니다. `ATLAS_ADMIN_CODE`와 `ATLAS_ROSTER`입니다. 기본값이 있는 관리자 코드는 이미
공개된 관리자 코드입니다.

## 실행

```bash
source .venv/bin/activate
export $(grep -v '^#' .env | xargs)
python -m uvicorn --factory src.server:create_app \
  --host 0.0.0.0 --port 8100 --proxy-headers --forwarded-allow-ips='*'
```

**5분 일찍** 켜세요. 첫 실행은 모델을 불러오고 UMAP의 numba 커널을 예열합니다. 수업 시작에
맞춰 켜면 그 비용이 그 학기 첫 의견 제출에 그대로 얹힙니다.

`GET /healthz`는 모델 캐시 키, 누적 의견 수, 사용 중인 투영 방식, 접속자 수를 보고합니다.
접근 코드는 절대 보고하지 않습니다.

### 강의실 접속

**먼저 내부망을 시도하세요.** 지연이 가장 낮고, 할당량도 제3자도 없습니다.
`http://<내 IP>:8100`과 QR 코드를 공유하면 됩니다. **전날** 같은 SSID의 휴대폰에서
반드시 시험해 보세요. 많은 캠퍼스·게스트 네트워크가 클라이언트 격리를 켜두는데, 이는
휴대폰↔노트북 통신을 조용히 차단하며 증상이 서버가 죽은 것과 똑같습니다.

**SSH 터널은 수업용 경로가 아닙니다.** `ssh -L 8100:127.0.0.1:8100 …`은 명령을 입력한
*그 노트북 한 대에만* 포트를 전달합니다. 강사가 SSH 너머로 지도를 미리 보는 데는 맞는
방법이고, 학생 휴대폰 30대에는 아무 역할도 하지 않습니다.

### 수업 당일

```bash
scripts/class.sh start     # 서버 + 터널, 둘 다 분리 기동하고 주소와 QR을 출력
scripts/class.sh url       # 주소와 QR 다시 보기 (슬라이드용)
scripts/class.sh status    # 명단/맥락, 입력 한도, 대기열, 모델/의미 분할 검토
scripts/class.sh backup    # 미처리 원문까지 포함한 비공개 SQLite 복구 백업
scripts/class.sh stop      # 접수/대기열 확인 후 백업하고 종료
```

5분 전에 실행하세요. 두 프로세스를 `setsid`로 띄우는 것이 이 스크립트의 핵심입니다.
평범한 셸에서 `cloudflared`를 실행하면 노트북이 절전에 들거나 와이파이가 끊기는 순간
터널이 죽고, 학생들이 보고 있던 주소가 수업 도중에 사라집니다. 서버는 그래도
살아남지만 터널은 아닙니다.

주소는 무작위 네 단어라 스캔하기엔 충분하고 강의실 뒤에서 읽기엔 가망이 없습니다. 그래서
`start`는 터미널에 QR을 찍고 슬라이드용으로 `.run/qr.png`도 만듭니다. QR을 띄우세요.
아무도 이 주소를 타이핑할 일이 없어야 합니다.

`ssh -L` 포워드는 이 과정에 **필요 없습니다.** 터널이 열리면 그 HTTPS 주소가 강사
노트북에서도 그대로 열리므로, 포워드가 할 일이 남지 않습니다.

짧은 `http://<호스트-IP>:8100`이 더 나아 보이지만 이 호스트에서는 잘못된 선택입니다. 주소가
공인 대역이라 `0.0.0.0` 바인딩은 강의실이 아니라 인터넷 전체에 앱을 공개하는 것이고, 평문
HTTP이며, 관리자 엔드포인트가 같은 포트에 함께 열립니다. 터널은 HTTPS이고 포트를 열지
않으며, QR이 주소 길이를 무의미하게 만듭니다.

**Cloudflare 임시 터널은 임시 접속 확인에 사용할 수 있습니다.** 계정 없이 HTTPS 주소를
만듭니다. 실제 수업에서는 브라우저 초안 접근을 유지할 수 있는 고정 HTTPS 주소를 권장합니다.

```bash
cloudflared tunnel --url http://localhost:8100
# → https://<무작위-단어>.trycloudflare.com   이 주소나 QR을 공유
```

이전 v1에서는 휴대폰 크기의 브라우저로 HTTPS·WebSocket 연결·제출 왕복을 확인했습니다.
v2를 실제 휴대폰과 수업 URL에서 검증한 결과는 아직 없습니다. 임시 터널은 재시작할 때
주소가 바뀌며, 새 출처(origin)에서는 이전 주소의 브라우저 저장소에 접근할 수 없습니다.

이 앱이 SSE 대신 WebSocket을 쓰는 이유이기도 합니다. `trycloudflare.com`은
`text/event-stream`을 버퍼링하므로 SSE는 터널을 통과하지 못합니다. 터널 트래픽은
HTTPS이므로 페이지는 그에 맞춰 `wss://`로 접속합니다.

임시 터널은 유휴 소켓을 100초쯤에 끊습니다. 하필 발표 중 조용한 구간입니다. 서버가 25초마다
하트비트를 보내고 클라이언트는 백오프로 재연결한 뒤 재동기화하므로 이미 처리되어 있습니다.
다만 그 코드가 왜 있는지는 알아둘 값어치가 있습니다.

## 관리자 모드

`/#admin`을 열고 접근 코드를 입력합니다. 코드는 UI 플래그가 아니라 **다른 웹소켓**을
엽니다. 두 엔드포인트가 서로 다른 직렬화를 쓰기 때문에, 참가자 연결이 관리자 연결이 되는
경로는 존재하지 않습니다.

관리자 전용 도구 네 가지입니다.

| | |
|---|---|
| **검색** | 의견 본문 부분 일치 검색. 일치 부분을 강조하며, 주차 필터·대상 강조·작성자 강조와 함께 겹쳐서 동작합니다 |
| **선택** | 지도 위 사각 또는 올가미 선택. 고른 의견이 읽을 수 있는 목록으로 나옵니다. 군집을 한 점씩 마우스로 짚는 대신 **이야기할 수 있게** 만드는 기능입니다 |
| **가까운 의견** | 점을 누르면 코사인 거리 기준 가장 가까운 8건. 이 워크숍의 질문을 점 하나 단위로 묻는 것입니다. 사람이 쓴 이 문장에 가장 가까이 떨어진 모델의 문장은 무엇인가? |
| **CSV 내보내기** | 전체 또는 현재 선택분을, 작성자와 좌표까지 포함해서 |

## Embedding Atlas 뷰어

지도와 나란히, 이 앱은 Apple의
[Embedding Atlas](https://github.com/apple/embedding-atlas) 뷰어 자체를 서빙합니다.
비슷하게 만든 것이 아니라 실제 컴포넌트입니다. 정렬 가능한 표, 칼럼마다의 분포 차트와
상호 교차 필터, SQL 조건식, 전문 검색, 그리고 밀도 등고선과 자동 군집 이름표를 갖춘
WebGPU 임베딩 뷰가 들어 있습니다.

현재 분석 버튼에는 사용할 수 없는 이유를 표시하며, 모든 화면 크기에서 강의실 지도를
유지합니다. 번들의 worker 경로는 수정·검증했지만, 실제 WebGPU 렌더링과 공통 필터·선택
동작은 출시 기준을 통과하지 못했습니다.

**Mosaic 애플리케이션이라 데이터를 받는 게 아니라 질의합니다.** 서버가 코퍼스를 담은
인메모리 DuckDB를 두고 `POST /data/query`로 답합니다. `embedding_atlas.server`의 엔드포인트를
명령 세 종류까지 그대로 따랐기 때문에 컴포넌트는 수정 없이 붙습니다. 재계산 때마다 웹소켓
브로드캐스트 **전에** 이 릴레이션을 갱신하므로 차트와 지도가 어긋나지 않습니다.

**데이터베이스는 둘이고, 엔드포인트는 SQL을 파싱하기 전에 하나를 고릅니다.** 질의를
작성하는 쪽이 클라이언트이므로 내보내는 쪽의 필터는 버틸 수 없습니다.
`SELECT reviewer_id FROM dataset`은 Mosaic 입장에서 평범한 요청입니다. 그래서 참가자용
데이터베이스에는 그 칼럼이 아예 없고, 요청하면 데이터가 아니라 DuckDB의 바인더 오류가
돌아옵니다. 관리자 코드는 URL이 아니라 요청 본문으로 전달됩니다. CSV 내보내기와 같습니다.

**AI/사람 구분은 모양에서 필터로 옮겨졌습니다.** `EmbeddingView`는 범주 색 하나만
인코딩하므로 지도의 두 번째 채널에 대응하는 것이 없습니다. 대신 `source` 차트가 대시보드
전체를 교차 필터합니다. `ai`를 누르면 다른 모든 차트와 표와 임베딩 뷰가 거기에 맞춰
좁혀집니다. 두 군집 비교가 범례가 아니라 클릭 한 번이 됩니다.

**번들 다시 만들기.** `web/vendor/`는 커밋되어 있고 수업 중에 받아오는 것은 없습니다.
버전을 바꾼 뒤에는:

```bash
cd frontend && npm install && npm run build   # -> ../web/vendor
```

Node는 이 빌드에만 필요하고 다른 곳에는 필요 없습니다. 서버는 실행하지 않습니다.

같은 패키지로 내보낸 CSV를 공식 CLI로 열 수도 있습니다. 서버가 전혀 필요 없습니다:

```bash
embedding-atlas feedback-atlas.csv --text text --x x --y y
```

## AI 의견 일괄 등록 (v2)

`scripts/seed_ai_opinions.py`는 **서버를 멈춘 상태에서 쓰는 관리자용 오프라인 가져오기**입니다. 다음 서버 시작 때 공통 작업자가 저장된 대기열을 처리합니다. 가져오기 스크립트는 모델을 적재하지 않으며 온라인 대량 등록 API를 제공하지 않습니다.

```bash
.venv/bin/python scripts/seed_ai_opinions.py comments.csv \
  --db atlas.db --roster roster.csv --reviewer instructor --dry-run
.venv/bin/python scripts/seed_ai_opinions.py comments.csv \
  --db atlas.db --roster roster.csv --reviewer instructor --segment
```

CSV/JSONL의 열은 `target_id,text[,source][,week]`입니다. 출처 기본값은 AI이며 행마다 바꿀 수 있습니다. 기본 모드는 각 행을 이미 나눈 의견 하나로 취급하고, `--segment`는 의미 분할을 요청합니다. 두 모드 모두 원문 공백·줄바꿈·유니코드 표현을 그대로 보존하고, 벡터와 좌표를 계산하기 전에는 공개 지도에 표시하지 않습니다. 원문 한 건은 최대 20,000 코드 포인트입니다.

파일 바이트·행 위치·최종 메타데이터·모드가 같으면 재실행해도 중복 등록되지 않습니다. 같은 문장이 서로 다른 행에 있으면 별도 원문입니다. 파일이나 메타데이터/모드를 바꾸면 새 가져오기로 취급합니다. 소유 권한 값은 무작위로 만들고 해시만 저장하며 출력하지 않습니다. 가져온 원문은 관리자 권한으로 관리합니다.

대기 한도는 300건이며 `--max-pending`으로 낮출 수 있습니다. 대기열이 차면 이미 받은 행은 보존하고 건너뛴 행을 알립니다. 대기열을 처리한 뒤 같은 파일과 옵션으로 다시 실행하세요. `--dry-run`은 DB를 열거나 만들지 않고 내용만 검사하며, 재실행 여부와 현재 대기열 용량은 확인하지 않습니다.

## 수업 준비·종료와 복구

현재 의미 분할의 수업 적용은 **NO-GO**입니다. 합성 보류 사례에서 단일 주제 보존율이 37.5%였고, 독립된 두 사람의 경계 판단도 없습니다. `class.sh start`는 검토 미승인 상태에서 수업 터널을 열지 않습니다. `ATLAS_ALLOW_UNREVIEWED_SEMANTICS=1 scripts/class.sh start`는 개발 확인용 예외일 뿐 수업 품질 승인이 아닙니다.

수업별 DB 하나와 고정 HTTPS 주소를 사용하세요. 임시 터널 주소가 바뀌면 새 주소에서 이전 주소의 브라우저 초안을 자동 복구할 수 없습니다. 이전 브라우저/주소에서 복구하거나 주소 변경 전에 초안 텍스트를 보존해야 합니다.

종료 순서는 관리자 화면에서 접수 닫기 → 대기/실패 원문 확인·복구 → 관리자 복구 CSV 내보내기 → `scripts/class.sh stop`입니다. 종료 스크립트는 대기열을 확인하고 비공개 SQLite 전체 백업을 만든 뒤 프로세스를 멈춥니다. `stop --force`도 백업은 만들지만 미해결 작업의 종료를 허용합니다. 아직 브라우저에서 전송되지 않은 글은 DB 백업으로 복구할 수 없습니다.

```bash
.venv/bin/python scripts/backup_database.py \
  --db atlas.db --out recovery/atlas-before-upgrade.db
```

SQLite 백업 API를 사용하므로 WAL에 반영된 내용, 미처리 원문, 수정 이력, 소유 권한 해시까지 일관되게 보존합니다. 출력 권한은 0600이며 기존 파일은 덮어쓰지 않습니다. 실행 중인 WAL DB의 본체 파일만 복사하지 마세요.

v1→v2 마이그레이션은 변경 전에 비공개 일관성 백업 `atlas.db.pre-v2-backup`을 만듭니다. 같은 이름이 있으면 시간 접미사를 붙인 새 복사본을 만들고, 성공한 마이그레이션은 실제 경로를 `meta.migration_backup_path`에 기록합니다. 복사본에서 먼저 확인하고 해당 코드 버전을 함께 보관하세요. 오래 열린 v1 브라우저는 새 초안 저장 기능을 자동으로 얻지 못하므로 미전송 글을 먼저 복사하고 진행 중 제출을 정리해야 합니다.

롤백 전에는 접수를 닫고 서버를 멈춘 뒤 현재 v2 전체 백업과 업그레이드 후 받은 모든 원문의 복구 내보내기를 보존하세요. 새로 받은 글을 어떻게 복구할지 정리한 다음, 맞는 구버전 코드와 별도 DB 파일을 사용합니다. 업그레이드된 DB를 덮어쓰지 않습니다.

```bash
.venv/bin/python scripts/backup_database.py \
  --db recovery/atlas-before-upgrade.db --out restored-v1.db
# 해당 구버전 코드의 ATLAS_DB를 restored-v1.db로 설정한 뒤 시작합니다.
```

## 학기가 끝난 뒤: 가명 아카이브

```bash
.venv/bin/python scripts/deidentify.py \
  --db atlas.db --roster roster.csv --out archive/ --targets
```

원본 DB는 읽기 전용으로 열고 원본에서 마이그레이션하지 않습니다. 모든 원문·수정·의견 연결을 보존한 `archive.db`, 현재 공개 의견의 `archive.csv`, 대기/실패/철회 원문도 포함하는 `originals.csv`를 만듭니다. 제출자 코드를 무작위로 바꾸고 원문/의견 식별자를 일관되게 다시 부여하며, 선택적으로 발표 대상도 가명 처리합니다. 재전송·소유 권한·요청·수정 작업 식별자와 비공개 메타데이터를 제거한 뒤 복사본을 압축·검증합니다. 정확한 원문은 SQLite에 보존하며 CSV에는 스프레드시트 수식 방지 처리가 들어갑니다.

`mapping.csv`는 별도로 보관하거나 파기할 명단 대응표입니다. 전체 복구 백업에는 권한 정보가 있지만 이 가명 아카이브에는 없으므로, 가명 아카이브를 수업 복구 DB로 사용하지 마세요. **가명 처리는 익명 처리가 아닙니다.** 본문, 시간, 남겨 둔 발표 정보로 사람을 식별할 수 있습니다. 이 스크립트는 본문의 신원 정보를 자동으로 제거하거나 연구 동의를 대신하지 않습니다.

## 테스트

```bash
cd feedback-atlas
.venv/bin/python -m pytest -q
```

CPU만 사용하고, 모델 가중치도 네트워크도 쓰지 않습니다. 저장소의 다른 부분과 같은
규칙입니다. 필수 의존성은 numpy와 pytest뿐이며, umap·fastapi·httpx·duckdb가 필요한 테스트는
실패가 아니라 건너뜁니다. 그래서 맨 인터프리터에서도 전체 suite가 돌아갑니다.

그중 둘은 알아둘 값어치가 있습니다. 보통은 "그러길 바라는" 수준에 그치는 것을 실제로
확인하기 때문입니다.

- `test_embedder_contract.py`는 `src.embedder`를 **별도 프로세스에서** import한 뒤 torch가
  `sys.modules`에 없음을 확인합니다. 이 규칙 하나가 이 suite를 오프라인으로 유지합니다.
  다른 테스트가 torch를 한 번 불러오고 나면, 같은 프로세스 안의 검사로는 누가 불러왔는지
  알 수 없습니다.
- `test_server_routes.py`는 참가자와 관리자를 **동시에** 하나의 서버에 붙여놓고, 한 번의
  브로드캐스트에서 관리자에게는 `reviewer_id`가 보이고 참가자에게는 보이지 않음을
  확인합니다. 이 누출이 살아 있는 동안에도 모듈별 테스트는 전부 통과했습니다. 두 채널이
  동시에 연결되어 있을 때만 존재하는 결함이기 때문입니다.
- `test_viewer_pipeline.py`는 실제 웹소켓으로 문장을 제출한 뒤 돌아가는 서버에 SQL로 그
  문장을 되묻습니다. 뷰어 쪽을 정직하게 검사하는 방법은 이것뿐입니다. HTTP로
  `reviewer_id`를 요청해 칼럼이 없어서 DuckDB가 거절하는지 확인하고, `/etc/passwd`를
  요청해 같은 거절을 확인합니다. duckdb·pyarrow·embedding-atlas가 없으면 건너뜁니다.

브라우저 동작 검사는 별도로 실행합니다. Playwright와 Chromium이 설치된 환경에서
`feedback-atlas/` 디렉터리의 `.venv/bin/python -m pytest tests/browser_viewer.py -q`를
실행하세요. 임시 데이터베이스와 로컬 서버에서 비활성화된 분석 버튼, 지도 유지, 실시간 명단
변경을 확인합니다. 전체 제출·복구·수정·내보내기 명령과 측정 결과는
[검증 보고서](docs/qa/feedback-atlas-release.md)에 있습니다. 브라우저 검사는 합성 데이터와
가짜 모델을 사용하며 의미 분할 품질은 별도로 평가합니다.

## 알려진 한계

- **신원 확인은 설계상 하지 않습니다.** 링크를 아는 사람이 동료의 ID를 알면 그 사람인 척
  쓸 수 있습니다. PRD가 로그인 없는 흐름을 받아들였으므로 이는 문서화된 잔여 위험입니다.
  다만 관리자 모드 작성자 조회의 신뢰도 상한이기도 하니, 성적과 관련된 어떤 판단에도
  기대기 전에 알고 있어야 합니다.
- **의견이 80건이 되는 순간 지도가 한 번 재배치됩니다.** 투영 방식이 PCA에서 UMAP으로
  바뀌기 때문이며, 두 방식은 구조적으로 다른 배치를 만들어 어떤 정렬로도 메울 수 없습니다.
  `ATLAS_PCA_UMAP_THRESHOLD`로 조정할 수 있으니, 발표 도중이 아니라 세션 사이에 의도적으로
  넘기세요.
- **AI/사람 구분은 자기 신고입니다.** 누구나 어느 쪽으로든 표시할 수 있으므로(PRD 5.2),
  AI-사람 비교의 신뢰도는 표시의 정확도만큼입니다. PRD 11이 지적한 사항이며 앱은 이를
  탐지하려 하지 않습니다.
- **데이터베이스는 로컬 디스크에 두세요.** SQLite의 WAL 모드는 NFS/SMB에서 신뢰할 수
  없습니다.

## 설계 노트

Apple의 [Embedding Atlas](https://github.com/apple/embedding-atlas)(MIT)는 여기서
세 가지로 쓰이며, 그중 마지막만 겉모습입니다.

**분석 방식 — 호출이 아니라 재현입니다.** `src/ea_projection.py`는 umap의
`nearest_neighbors`로 근사 k-NN 그래프를 만들고 그것을 `precomputed_knn`으로 넘겨
UMAP을 학습시킵니다. `embedding_atlas.projection`이 쓰는 것과 같은 순서, 같은
파라미터입니다. 다만 그쪽 함수를 부르지는 않습니다. 그 함수는 데이터프레임을 만들고,
입력을 키로 하는 파일 캐시로 감싸고, 학습된 reducer를 버립니다. reducer가 없으면 의견이
하나 들어올 때마다 전체를 다시 학습해야 하고, 발표 중에 읽히는 지도는 그 비용을 감당할 수
없습니다. 그래서 알고리즘과 기본값은 그쪽 것이고, 호출은 이쪽 것입니다. 그 학습이 만들어낸
그래프가 `neighbors` 칼럼을 채웁니다.

혼동하기 쉬우니 분명히 적어 둡니다. 실행 경로에서 실제로 도는 상위 패키지의 파이썬 코드는
`/data/query`가 차트 질의에 답할 때 쓰는 Arrow 작성기 하나뿐입니다.

**뷰어와 렌더러.** `web/vendor/`에 번들로 넣고 서버의 DuckDB에 붙였습니다. 위 절을
보세요.

**시각 언어** — 손으로 쓴 지도 쪽입니다. 슬레이트 바탕, 흰색/검정 카드, 1px 테두리,
단일 파란 강조색, d3의 `category10`, 반투명 툴팁, 후광 처리된 지도 이름표. 한국어를
위한 두 가지 변경: 의견 본문을 13px이 아닌 14px/1.6로, 그리고
`word-break: keep-all` — 없으면 브라우저가 한국어를 단어 중간에서 끊습니다.

`web/atlas.js`의 손으로 쓴 산점도는 남아 있고, 중복이 아닙니다. `EmbeddingView`가
채널 하나를 인코딩하는 자리에서 둘을 인코딩하고, WebGPU 없이 동작하며, 의견이 도착하는
순간에 애니메이션을 줍니다. 휴대폰이 받는 화면이자, 뷰어를 그릴 수 없는 노트북이 받는
화면입니다.

CDN도, 웹폰트도, 수업 중에 받아오는 것도 없습니다. 프런트엔드는 여전히 `web/`에서
그대로 서빙되는 순수 ES 모듈이고, 번들러는 개발 시점에 `frontend/`에서 한 번만 돌며 그
결과물은 저장소에 커밋되어 있습니다.
