...
`USE_PRECOMPUTED = True`로 바꾸세요: **W7-W9**의 모든 비대화형 그래프가
[`precomputed/`](precomputed/)에 커밋된 산물로부터 GPU도, 8 GB 다운로드도 없이
렌더링됩니다(각각 *"replayed from cache"* 라벨이 붙습니다). 기본값은 `False`(라이브
...
| **Teacher-forced Δ log-lik** | 기준 캡션을 `answer`로 태그해 다시 입력하고, 같은 녹아웃 아래에서 모델이 그 캡션을 토큰별로 얼마나 덜 믿게 되는지 점수화. | 문자열 diff의 **측정** 버전: 연속적(작은 효과도 보임)이고 결정론적. |
...
모델이 모든 위치에 붙이는 여섯 가지 토큰 타입(모달리티):
...
> 모델이 자기 캡션을 *덜* 믿게 되었다는 뜻입니다. 이것이 W9 문자열 diff의 인과적 짝이며,
> `answer` source가 의미를 갖게 되는 곳입니다.
...
판정. 워크시트의 마지막 블록은 W11 프로젝트가 채점되는 바로 그 (효과, 대조군) 질문입니다.
...
6. **시각 vs 청각, 정량화.** 🎯 티처 포싱 섹션에서 같은 클립·프롬프트로 `answer → audio`와
   ...
   이것은 각각에 대해 모델이 자기 캡션에 대한 믿음을 **몇 nat이나** 잃는지 묻습니다. 이진
   diff와 연속 측정은 이 클립이 어느 감각에 기대는지에 대해 같은 답을 내나요?
7. **캡션의 오디오 근거는 어디에 사는가.** `answer → audio`를 `[0, 12)`, `[12, 24)`,
   `[24, 36)` 구간으로 돌려 보세요. 어느 레이어 대역을 잘랐을 때 캡션이 믿음을 가장 많이
   잃나요? 실험 2와 비교해 보세요 — 오디오를 표현에 *융합하는* 레이어와, *답변*이 오디오를
   읽어 가는 레이어가 일치하나요?
8. **의미 있는 널.** `assets/02321_silent.mp4`에 `answer → audio`를 돌려 보세요 — 샘플
   클립과 같은 프레임이지만 사운드트랙이 디지털 무음입니다. 오디오 토큰은 존재하지만 Δ는
   ≈ 0에 머물러야 합니다: 신호를 나르지 않는 경로를 잘라도 모델의 믿음은 줄지 않아야
   하니까요. *소리 있는* 클립의 Δ가 뚜렷이 더 크지 않다면, 그것은 측정에 대해 무엇을
   말해 주나요? *(이 실험 하나가 이 과목의 논지입니다: 효과는 실패할 수 있는 대조군 옆에
   있을 때만 해석 가능합니다.)*
